...
for ticker, senti in product(tickers, ['ml_sentiment', 'vader']):
    with open(root_path + f"20_outputs/benchmarks/{ticker}/{senti}/LogisticRegStats.log", 'r') as f:
        s = f.read()
    score = [float(x) for x in re.findall("(?<=TEST Accuracy = )(.*)\n", s)[-1:]][0]
    results.append((ticker, senti, score))


df = pd.DataFrame(results, columns=['ticker', 'senti', 'logreg_score']).set_index('ticker')
# df
...
#%%
final.round(3).to_csv(root_path + "30_results/LogisticRegressionBenchmark.csv", sep=';')
